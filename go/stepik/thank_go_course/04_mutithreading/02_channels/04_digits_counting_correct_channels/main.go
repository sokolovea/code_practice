...
func countWords(in chan string, out chan pair) {
	for word := range in {
		wordDigitsCount := countDigits(word)
		out <- pair{word: word, count: wordDigitsCount}
	}
	close(out)
}
...
func fillStats(in chan pair) counter {
	stats := counter{}
	for wordProcessingResult := range in {
		stats[wordProcessingResult.word] = wordProcessingResult.count
	}
	return stats
}
