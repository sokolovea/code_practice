...
func submitWords(next nextFunc, outputChan chan string) {
	for {
		word := next()
		outputChan <- word
		if word == "" {
...
func countWords(inputChan chan string, outputChan chan pair) {
	for {
		word := <-inputChan
		outputChan <- pair{word, countDigits(word)}
		if word == "" {
			break
		}
	}
...
func fillStats(inputChan chan pair) counter {
	stats := counter{}
	for {
		result := <-inputChan
		if result.word == "" {
