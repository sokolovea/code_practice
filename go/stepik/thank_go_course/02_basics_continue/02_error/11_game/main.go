...
func (ise invalidStepError) Error() string {
	return fmt.Sprintf("%s", ise.message)
}

// func (ise invalidStepError) Unwrap() error {
// 	return ise.err
// }
...
func (neoe notEnoughObjectsError) Error() string {
	return fmt.Sprintf("%s", neoe.message)
}

// func (neoe notEnoughObjectsError) Unwrap() error {
// 	return neoe.err
// }
...
func (clee commandLimitExceededError) Error() string {
	return fmt.Sprintf("%s", clee.message)
}

// func (clee commandLimitExceededError) Unwrap() error {
// 	return clee.err
// }
...
func (olee objectLimitExceededError) Error() string {
	return fmt.Sprintf("%s", olee.message)
}

// func (olee objectLimitExceededError) Unwrap() error {
// 	return olee.err
// }
...
func (p *player) do(cmd command, obj thing) error {
	...
	case eat:
		if p.nEaten > 1 {
			return commandLimitExceededError{command: eat, limit: 1, message: "you don't want to eat anymore"}
		}
		p.nEaten++
	case take:
		if p.has(obj) {
			return objectLimitExceededError{object: obj, limit: 1, message: fmt.Sprintf("you already have a %s", obj)}
		}
		p.inventory = append(p.inventory, obj)
	case talk:
		if p.nDialogs > 0 {
			return commandLimitExceededError{command: talk, limit: 0, message: "you don't want to talk anymore"}
		}
		p.nDialogs++
	}
	return nil
...
func (g *game) execute(st step) error {
	...
	if !st.isValid() {
		return gameOverError{err: invalidStepError{command: st.cmd, object: st.obj.name}, nSteps: g.nSteps}
	}
	...
	if st.cmd == take || st.cmd == eat {
		if !g.has(st.obj) {
			return gameOverError{err: notEnoughObjectsError{object: st.obj.name}, nSteps: g.nSteps}
		}
	...
	if err := g.player.do(st.cmd, st.obj); err != nil {
		return gameOverError{err: err, nSteps: g.nSteps}
	}
...
func giveAdvice(err error) string {
	var gameOverError gameOverError
	if errors.As(err, &gameOverError) {
		var invalidStepError invalidStepError
		var notEnoughObjectsError notEnoughObjectsError
		var commandLimitExceededError commandLimitExceededError
		var objectLimitExceededError objectLimitExceededError
		if errors.As(err, &invalidStepError) {
			return fmt.Sprintf("things like '%s %s' are impossible", invalidStepError.command, invalidStepError.object)
		}
		if errors.As(err, &notEnoughObjectsError) {
			return fmt.Sprintf("be careful with scarce %ss", notEnoughObjectsError.object)
		}
		if errors.As(err, &commandLimitExceededError) {
			if commandLimitExceededError.command == eat {
				return "eat less"
			}
			if commandLimitExceededError.command == talk {
				return "talk to less"
			}
		}
		if errors.As(err, &objectLimitExceededError) {
			return fmt.Sprintf("don't be greedy, %v %s is enough", objectLimitExceededError.limit, objectLimitExceededError.object)
		}
	}
	return ""
}
